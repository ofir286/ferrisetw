...
impl UserTrace {
    ...
    /// `EnableTraceEx2`. It is different from the `TraceHandle` returned by `TraceBuilder::start`,
    /// which is a `PROCESSTRACE_HANDLE` from `OpenTraceW` and cannot be used with `EnableTraceEx2`.
